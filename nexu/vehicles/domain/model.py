...
class Model:
    ...
    @classmethod
    def create_model(cls, model_id: int, name: str, average_price: int, brand_name: str):
        return cls(
            model_id=model_id,
            name=name,
    ...
    def verify_if_model_exists(name: str, models: List['Model']):
        filtered_models = list(filter(lambda model: model.name.upper() == name.upper(), models))

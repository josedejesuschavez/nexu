...
class JSONModelsRepository(ModelsRepository):
    ...
    def get_all_models(
        ...
        models = [
            {"id": i["id"], "name": i["name"], "average_price": i["average_price"]}
            for i in data
        ]
        return models
    ...
    def get_models_by_brand_name(self, brand_name: str):
        ...
        filtered_data = list(
            filter(
                lambda model: model.get("brand_name").upper() == brand_name.upper(),
                data,
            )
        )
        models = [
    ...
    def get_model_by_id(self, model_id: int) -> Model:
        ...
        if filtered_models:
            return Model.create_model(
                model_id=filtered_models[0].get("id"),
                name=filtered_models[0].get("name"),
                average_price=filtered_models[0].get("average_price"),
                brand_name=filtered_models[0].get("brand_name"),
            )

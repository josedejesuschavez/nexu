...
class JSONBrandsRepository(BrandsRepository):
    ...
    def get_all_brands(self) -> List[Brand]:
        ...
        brands = [
            Brand.create_brand(
                brand_id=i["id"], name=i["nombre"], average_price=i["average_price"]
            )
            for i in data
        ]
        return brands
    ...
    def get_brand_by_id(self, brand_id: int) -> Brand:
        ...
        if filtered_brands:
            return Brand.create_brand(
                brand_id=filtered_brands[0].get("id"),
                name=filtered_brands[0].get("nombre"),
                average_price=filtered_brands[0].get("average_price"),
            )
